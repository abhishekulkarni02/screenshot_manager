...
window.CaptureAPI = (function() {
        ...
        chrome.tabs.captureVisibleTab(
            null, {format: 'png'}, function(dataURI) {
                if (dataURI) {
                    ...
                    image.src = dataURI;//manage can be added here.
                }
            });
    }
    ...
    function getBlobs(screenshots) {
        ...
        if(!localStorage.key(0)){
            localStorage.setItem("preview1",dataURI);
        }

        else if(!localStorage.key(1)){
            localStorage.setItem("preview2",dataURI);
        }

        else if(!localStorage.key(2)){
            localStorage.setItem("preview3",dataURI);
        }

        else if(!localStorage.key(3)){
            localStorage.setItem("preview4",dataURI);
        }

        else if(!localStorage.key(4)) {
            localStorage.setItem("preview5",dataURI);
        }
        else{
            exc=localStorage.getItem("preview1");
            now=localStorage.getItem("preview2");
            localStorage.setItem("preview2",exc);
            exc=localStorage.getItem("preview3");
            localStorage.setItem("preview3",now);
            now=localStorage.getItem("preview4");
            localStorage.setItem("preview4",exc);
            localStorage.setItem("preview5",now);
            localStorage.setItem("preview1",dataURI);
            ...
            var byteString = atob(dataURI.split(',')[1]);//manage can be added here.
